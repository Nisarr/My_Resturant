...
const AnalyticsPage = () => {
  ...
  // Build daily summaries from orders
  ...
  const dailySummaries = Object.values(dailyMap).sort((a, b) => a.date.localeCompare(b.date)).slice(-14);

  // Item popularity from order items
  const itemCounts: Record<string, number> = {};
  orders.forEach(o => o.items.forEach(i => {
    itemCounts[i.menu_item_name] = (itemCounts[i.menu_item_name] || 0) + i.quantity;
  }));
  const popularityData = Object.entries(itemCounts)
  ...
  const exportCSV = () => {
    ...
    const rows = dailySummaries.map(d => `${d.date},${d.revenue.toFixed(2)},${d.orders}`).join('\n');
  ...
  return (
    <div className="space-y-6">
      ...
      {orders.length === 0 ? (
        <p className="text-center text-muted-foreground py-10">No order data yet. Create some orders to see analytics!</p>
      ) : (
        <>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={280}>
                    <AreaChart data={dailySummaries.map(d => ({ date: d.date.slice(5), revenue: d.revenue }))}>
                      <defs>
            ...
            <Card className="border-0 shadow-sm">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={dailySummaries.map(d => ({ date: d.date.slice(5), orders: d.orders }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(36, 15%, 88%)" />
                    <XAxis dataKey="date" tick={{ fontSize: 12 }} />
                    <YAxis tick={{ fontSize: 12 }} />
                    <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }} />
